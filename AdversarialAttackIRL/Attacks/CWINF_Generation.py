...
from Methods.CWINF import CWINFAttack


class CWINFGeneration(Generation):
    def __init__(self,
                 dataset, model, model_depth, dataset_type, pretrained,
                 clean_data_root, ae_save_root, model_root, device, name,
                 attack_batch_size, kappa, init_const, lr, binary_search_steps, max_iterations, lower_bound,
                 upper_bound, **kwargs):
        super(CWINFGeneration, self).__init__(dataset, model, model_depth, dataset_type, pretrained,
    ...
    def _generate(self):
        attacker = CWINFAttack(model=self.model, kappa=self.kappa, init_const=self.init_const, lr=self.lr,
...
def main(args):
    device = init(args)
    cw_inf = CWINFGeneration(**vars(args), device=device)
    cw_inf.generate()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--name', type=str, default='CWINF')
    ...
    parser.add_argument('--init_const', type=float, default=0.1,
                        help="the initial value of const c in the binary search.")
    parser.add_argument('--lr', type=float, default=2e-2, help="the learning rate of gradient descent.")
    parser.add_argument('--max_iterations', type=int, default=1000, help='maximum iteration')
